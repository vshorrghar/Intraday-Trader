"""Swing trading dashboard JSON writer."""
...
def write_swing_dashboard(db, config=None, dashboard_dir: str = "dashboard") -> None:
    """Write swing positions to dashboard JSON for the web UI."""
    os.makedirs(os.path.join(dashboard_dir, "api"), exist_ok=True)
    output_path = os.path.join(dashboard_dir, "api", "swing_latest.json")

    all_positions = db.get_swing_positions()
    ...
    logger.info("Swing dashboard updated: %s", output_path)

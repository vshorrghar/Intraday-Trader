...
class FnO_Position_Monitor:
    ...
    def monitor_cycle(self, current_time: datetime | None = None) -> dict:
        ...
        for strat in open_strategies:
            ...
            if new_status and new_status != status:
                ...
                # No exit triggered — check if adjustment needed (Phase 4)
                try:
                    from fno.adjustment_engine import should_adjust, execute_adjustment
                    # Get spot price from chain (use first chain available)
                    spot_price = 0.0
                    chain_data = {}
                    if positions:
                        # Try to get spot from positions
                        for pos in positions:
                            if pos.get("index_name") == strat.get("index_name"):
                                spot_price = float(pos.get("spot_price", 0))
                                break

                    if spot_price > 0:
                        adj_instruction = should_adjust(
                            strategy=strat,
                            current_spot=spot_price,
                            current_chain=chain_data,
                            db=self.db,
                            vix=0,  # VIX checked at session level, not per-cycle
                        )
                        if adj_instruction:
                            result = execute_adjustment(
                                strategy_id, adj_instruction, self.broker, self.db,
                            )
                            if result.get("success"):
                                logger.info(
                                    "Strategy %d ADJUSTED: %s",
                                    strategy_id, adj_instruction.get("action"),
                                )
                                self.db.insert_audit_log(
                                    "FNO_ADJUSTMENT",
                                    json.dumps({
                                        "strategy_id": strategy_id,
                                        "action": adj_instruction.get("action"),
                                        "trigger": adj_instruction.get("trigger_reason"),
                                    }),
                                )
                except Exception as adj_err:
                    logger.debug("Adjustment check failed for strategy %d: %s", strategy_id, adj_err)
    ...
    def _evaluate_exit_conditions(
        ...
        """Evaluate all exit conditions using Phase 5 improved exit rules.

        Phase 5: Uses rules_strategy_engine.EXIT_RULES for proper profit targets.
        Holds positions longer for meaningful theta capture.
        """
        status = strat["status"]
        strategy_type = strat.get("strategy_type", "").upper()
        max_profit = float(strat.get("max_profit", 0))

        # Import exit rules from rules engine
        from fno.rules_strategy_engine import get_exit_rules
        exit_rules = get_exit_rules(strategy_type)

        # 1. Force exit at force_exit_time (unchanged — safety net)
        try:
        ...
        # 2. Time-based exit (DTE check) — exit N days before expiry
        try:
            ...
            if legs_json:
                ...
                time_exit_dte = exit_rules.get("time_exit_dte", 1)
        ...
        # 3. Premium-based stop loss for sold strategies
        ...
        if is_selling and entry_premium > 0:
            # Loss exit: premium expanded beyond threshold
            loss_multiplier = exit_rules.get("loss_exit_multiplier", 1.5)
            sl_threshold = entry_premium * (1 + loss_multiplier)
            if current_premium > sl_threshold:
                return "STOPPED_OUT"

        # 4. Profit target (Phase 5: uses proper targets from EXIT_RULES)
        if status == "OPEN" and max_profit > 0:
            profit_target_pct = exit_rules.get("profit_target_pct", 50) / 100.0
            ...
            target_amount = max_profit * profit_target_pct
            if current_profit >= target_amount:
                return "CLOSED"  # Full close at profit target (not partial)

        # 5. Full premium decay (premium near zero = max profit captured)
        if is_selling and current_premium <= entry_premium * 0.05:
            return "CLOSED"
    ...
    def _compute_current_premium(self, strat: dict, positions: list[dict]) -> float:
        """Compute current combined premium for a strategy using REAL chain prices.

        Phase 5: Replaced random theta simulation with real Dhan option chain lookup.
        Uses option_chain_cache (5-min TTL) for price freshness.

        For live mode: uses actual position LTPs from broker.
        For paper mode: fetches real chain prices (same as live would see).
        """
        entry_premium = abs(float(strat.get("net_premium", 0)))

        # Try to compute from real chain prices (both paper and live)
        try:
            ...
            index = strat.get("index_name", "NIFTY")
            expiry = legs[0].get("expiry_date", "") if legs else ""

            # Use pnl_calculator's get_current_premium for each leg
            from fno.pnl_calculator import get_current_premium
            from fno.option_chain_cache import fetch_option_chain_with_cache

            # Build chain fetcher — use broker if available, else cache
            def get_chain(idx, exp):
                if self.broker and hasattr(self.broker, "get_option_chain"):
                    from fno.option_chain_cache import get_cached_chain, save_chain_to_cache
                    cached = get_cached_chain(idx, exp or "current")
                    if cached:
                        return cached
                    try:
                        import time
                        time.sleep(2)  # Rate limit
                        chain = self.broker.get_option_chain(idx)
                        if chain and chain.get("strikes"):
                            save_chain_to_cache(idx, exp or "current", chain)
                            return chain
                    except Exception:
                        pass
                return {}

            # Compute current premium from real chain
            total_current = 0.0
            all_priced = True
            ...
            for leg in legs:
                ...
                txn = leg.get("transaction_type", "SELL")
                num_lots = int(leg.get("num_lots", 1))
                lot_size = int(leg.get("lot_size", 25))
                qty = num_lots * lot_size

                current_ltp = get_current_premium(strike, opt_type, expiry, index, get_chain)

                if current_ltp is None:
                    all_priced = False
                    break

                # For the combined position premium:
                # SELL legs contribute positive premium (we collected it)
                # BUY legs contribute negative premium (we paid it)
                if txn == "SELL":
                    total_current += current_ltp * qty
                else:
                    total_current -= current_ltp * qty

            if all_priced:
                # Return absolute current premium (comparable to entry_premium)
                return abs(total_current)

        except Exception as e:
            logger.debug("Real chain pricing failed, falling back to entry: %s", e)

        # Fallback: return entry premium (no change = no exit trigger)
        return entry_premium
...
def _update_fno_dashboard(profile: str, db_path: str) -> None:
    ...
    total_pnl = sum(float(s["realized_pnl"] or s.get("corrected_pnl") or 0) for s in strategies)

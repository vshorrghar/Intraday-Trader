...
def authenticate_broker(
    ...
    # --- Check for SHARED token (per client_id, generated by pre_auth_dhan.py) ---
    expected_client_id = str(broker_config.get("client_id", ""))
    if expected_client_id and name == "dhan":
        shared_token_file = Path("config") / f".dhan_token_{expected_client_id}.json"
        if shared_token_file.exists():
            try:
                import json as _json
                shared_data = _json.loads(shared_token_file.read_text())
                if shared_data.get("date") == __import__("datetime").date.today().isoformat():
                    shared_access_token = shared_data.get("access_token", "")
                    if shared_access_token:
                        config_with_token = {**broker_config, "access_token": shared_access_token}
                        client = broker_factory(name, config_with_token)
                        logger.info("Using SHARED token for client_id %s (from pre_auth_dhan.py)", expected_client_id)
                        return client
            except Exception as e:
                logger.warning("Shared token file read failed: %s — falling through to per-profile auth", e)

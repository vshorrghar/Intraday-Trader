...
### V1 vs V2 Status:
- **V1 (LLM-based):** PAUSED on live. Still runs on vishal/neha paper.
- **V2 (rules-based):** LIVE on vishal-live-v2 since May 24. Profile: vishal-live-v2.yaml
...
### P0 — Tuesday May 26 (market open)
- Verify V2 fired correctly: `tail -f logs/cron_vishal_live_v2.log`
- Check: did it use V6/V4 signals? Did it skip flat market?
- Check: no V1 fallback (grep for "V1" in log)

### P1 — Strategy expansion
- Build Strategy 2: VWAP reclaim (11:00-13:00 IST window)
- Build Strategy 3: Trend continuation (13:00-14:30 IST)
- Both go into selector_v2.py as additional signal generators

### P2 — Swing paper deployment
- Fetch 2+ years of daily data (extend from_date to 2024-05-01)
- Re-run CRABEL backtest on 12+ months
- If WR >= 45% and PF >= 1.3 → wire to cron for paper

### P3 — F&O validation
- Target: 30 clean paper trades with accurate P&L
- Currently at 27 strategies — need 3 more clean ones
- Then evaluate: is Iron Condor profitable after charges?

### P4 — Capital scaling
- If V2 shows 55%+ WR over 50 trades → scale to Rs.50K
- If swing CRABEL confirms → deploy with Rs.1L paper
- Never scale without backtest proof
...
# Check V2 ran today
tail -20 logs/cron_vishal_live_v2.log
grep "V2\|V6\|V4\|SKIP\|signal" logs/cron_vishal_live_v2.log | tail -10
...
### What to paste:
This entire document (MASTER_RESUME_2026_05_25.md) at session start.
